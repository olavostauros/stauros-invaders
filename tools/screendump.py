...
Usage:
    ...
    python3 tools/screendump.py --hold 16 --boom      # dump a frame with a burst alight
    python3 tools/screendump.py --hold 20 --lives 3 --bonus  # a saucer's bonus, once one is hit
...
def capture(hold, frames, state, ufo, lives, clear, boom, bonus):
    ...
    probe = re.sub(r"local PROBE_BOOM = 0", f"local PROBE_BOOM = {boom}", probe)
    probe = re.sub(r"local PROBE_BONUS = 0", f"local PROBE_BONUS = {bonus}", probe)
...
def main():
    ...
    clear = arg("--clear", 0)
    boom = 1 if "--boom" in sys.argv else 0
    bonus = 1 if "--bonus" in sys.argv else 0
    print(f"gamepad mask {hold} held, dumping frame {frames}"
          ...
          + (" or the first one with an explosion alight" if boom else "")
          + (" or the first one showing a saucer's bonus" if bonus else "") + "\n")
    rows = parse(capture(hold, frames, state, ufo, lives, clear, boom, bonus))
    if "--raw" in sys.argv:
