...
local function probe()
  ...
  local PROBE_BOOM = 0
  local PROBE_BONUS = 0
  -- The cart's BONUS_FRAMES minus its EXPLOSION_FRAMES, repeated here for the reason the
  -- saucer's width is: a burst worth something shows the sprite until its timer falls to
  -- this and the number afterwards, and it is the number this waits for.
  local PROBE_BONUS_TIMER = 36
  ...
  function TIC()
    ...
    -- A burst is up for twelve frames after a kill nothing can schedule: which invader the
    -- shot reaches depends on where the fleet has marched to, so this waits for one the way
    -- PROBE_UFO waits for a saucer rather than counting frames to it.
    if PROBE_BOOM == 1 then
      local burning = false
      for _, burst in ipairs(game.explosions) do
        if burst.timer > 0 then burning = true end
      end
      if not burning then return end
    end
    if PROBE_BONUS == 1 then
      local showing = false
      for _, burst in ipairs(game.explosions) do
        if burst.bonus > 0 and burst.timer > 0 and burst.timer <= PROBE_BONUS_TIMER then
          showing = true
        end
      end
